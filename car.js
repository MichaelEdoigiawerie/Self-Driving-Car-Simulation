class Car {
    constructor(x, y, width, height, controlType, maxSpeed = 3, color = "red") {
        ...
        this.useBrain = controlType == "Artificial Intelligence";
    ...
    draw(context, drawSensor = false) {
        if (this.sensor && drawSensor) this.sensor.draw(context);

        context.save();
        context.translate(this.x, this.y);
        context.rotate(-this.angle);
        if (!this.damaged) {
            context.drawImage(this.mask, -this.width / 2,
            -this.height / 2,this.width, this.height);
            context.globalCompositeOperation = "multiply";
        }

        context.drawImage(this.image, -this.width / 2,
        -this.height / 2,this.width, this.height);
        context.restore();
    }
    ...
    #createPolygon() {
        const points = [];
        const rad = Math.hypot(this.width - 10, this.height - 10) / 2;
        const alpha = Math.atan2(this.width - 10, this.height - 10);
